...
namespace ENTICourse.IK
{
    ...
    public class InverseKinematics : MonoBehaviour
    {
        ...
        [Range(0, 1f)]
        public float DeltaGradient = 0.2f; // Used to simulate gradient (degrees)
        [Range(0, 100f)]
        ...
        public float CalculateGradient(Vector3 target, float[] Solution, int i, float delta)
        {
            ...
            float gradient = 0;
            //target és es target
            //Solution són els angles
            //i és l'angle al qual volem accedir Solution[i]
            //delta és l'espai entre iteracions en la funcio

            float initDistance = DistanceFromTarget(target, Solution);
            Solution[i] += delta;

            float nextDistance = DistanceFromTarget(target, Solution);
            gradient = (nextDistance - initDistance)/delta;
